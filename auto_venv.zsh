#!/usr/bin/env zsh
# auto_venv.zsh - Zsh version with multi-environment support
...
# Declare associative array for environments (Zsh syntax)
typeset -gA AUTO_VENV_ENVIRONMENTS
...
__parse_auto_venv_file() {
  ...
  if [[ -z "$AUTO_VENV_DEFAULT" && ${#AUTO_VENV_ENVIRONMENTS[@]} -gt 0 ]]; then
    # In Zsh, get first value from associative array
    AUTO_VENV_DEFAULT="${AUTO_VENV_ENVIRONMENTS[${(k)AUTO_VENV_ENVIRONMENTS}[1]]}"
  fi
...
__validate_venv() {
  ...
  if [[ ! -f "$venv_path/pyvenv.cfg" ]]; then
    ...
    # Use Zsh glob with null_glob option
    setopt local_options null_glob
    local python_libs=("$venv_path"/lib/python*)
    
    if [[ ! -f "$venv_path/lib/python2.7/site.py" && ${#python_libs[@]} -eq 0 ]]; then
      return 1
    fi
  fi
...
__find_auto_venv_file() {
  ...
  while [[ "$current_dir" != "/" ]]; do
    ...
    current_dir="${current_dir:h}"  # Zsh way to get parent directory
  done
  ...
  if [[ "$1" == "--new" || "$1" == "--add" ]] ; then
    ...
    for python_exec in "$AUTO_VENV_PYTHON_SEARCH_PATH"/python*; do
      if [[ -x "$python_exec" && "$python_exec" =~ python[23](\.[0-9]+)?$ ]]; then
        local version_name
        version_name="${python_exec:t}"  # Zsh way to get basename
    ...
    VENV_MODULE="venv"
    read -r "PYTHON_VERSION?Python version to use? "
    ...
    read -r "VENV_PATH?Virtual environment path [.venv_${env_name//[.\/]/_}]: "
    if [[ -z "$VENV_PATH" ]] ; then
    ...
    for env in ${(k)AUTO_VENV_ENVIRONMENTS}; do
      local marker=""
